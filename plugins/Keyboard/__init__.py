...
class Keyboard(eg.PluginBase):
    ...
    def onKeyPress(self, event):
        codes = event.name.replace(' ', '_')
        num = event.scan_code.__str__()
        print ('keyboard event ' + event.name + '(' + event.scan_code.__str__() + ')')
        eg.TriggerEvent(num, prefix='Keypress.Key_Code')
        eg.TriggerEvent(codes, prefix='Keypress.Event')
    ...
    def KeyboardCallback(self, codes, num, lastNum):
        print('original callback with keyboard code {}, num {}, lastNum {}'.format(codes,num, lastNum))
        if codes == "":
            self.EndLastEvent()
        else:
            if num >= lastNum:
